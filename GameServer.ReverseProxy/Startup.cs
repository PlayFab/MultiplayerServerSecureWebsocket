...
public class Startup
{
    ...
    public void Configure(IApplicationBuilder app, IConfiguration configuration, IHttpForwarder forwarder)
    {
        ...
        app.UseEndpoints(endpoints =>
        {
            ...
            endpoints.Map("/{sessionId:guid}/{**forwardPath}", async context =>
            {
                ...
                if (!Guid.TryParse(routeValues["sessionId"]?.ToString(), out var sessionId))
                {
                ...
                try
                {
                    serverEndpoint = await detailsFactory.GetServerEndpoint(sessionId);
                }
                catch (Exception)
                {
...
/// </summary>
/// <example><c>/{sessionId}/some/path?test=true</c> is mapped to <c>{serverUrl}/some/path?test=true</c></example>
internal class GameServerRequestTransformer : HttpTransformer
{
    public override async ValueTask TransformRequestAsync(HttpContext httpContext,
        HttpRequestMessage proxyRequest, string serverEndpoint, CancellationToken cancellationToken)
    {
        await base.TransformRequestAsync(httpContext, proxyRequest, serverEndpoint, cancellationToken);
